...
const upload = multer({
  ...
  fileFilter: function (req, file, cb) {
    const fileTypes = /jpeg|jpg|png|svg|gif|pdf|doc|docx|ppt|pptx/;
...
const uploadSingleFile = async (file) => {
  const uploadParams = {
    ...
    Key: `job-application-attachments/${file.originalname}`, // Construct the file key
...
const uploadMultipleDocuments = async (req, res, next) => {
  upload.array('files', 5)(req, res, async (err) => { // Allows up to 5 files
    ...
    try {
      const uploadPromises = req.files.map(file => uploadSingleFile(file));
...
module.exports = uploadMultipleDocuments;
